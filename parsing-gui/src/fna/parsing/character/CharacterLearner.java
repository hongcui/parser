...
//partial rewrite of categorizeTerms.pl
...
public class CharacterLearner  {
	...
	public CharacterLearner(String database, String glossaryfilepath) {
		...
		try{
			if(conn == null){
				...
				stmt.execute("create table if not exists learnedstates (state varchar(100) NOT NULL PRIMARY KEY, count int(4))");
	...
	private void bootstrap(){
		/*try{
			Statement stmt = conn.createStatement();
			stmt.execute("create table if not exists bootstrap (state1 varchar(100), character1 varchar(200), PRIMARY KEY (state1, character1))");
		}catch(Exception e){
			e.printStackTrace();
		}*/
	...
	public void markupCharState(){
		...
		try{
			Statement stmt = conn.createStatement();
			ResultSet rs = stmt.executeQuery("select clauseid, tag, clause, charsegment from clause");
			while(rs.next()){
				...
				stmt1.execute("update clause set attributes = \""+attributes+"\" where clauseid = "+clauseid);
			}
	...
	private void assembleDescription(){
		try{
			Statement stmt = conn.createStatement();
			ResultSet rs = stmt.executeQuery("select filename, endindex from fileclauselink");
			int start = 0;
			while(rs.next()){
	...
	public String getMarkedDescription(String filename){
		try{
			Statement stmt = conn.createStatement();
			ResultSet rs = stmt.executeQuery("select filename, endindex from fileclauselink where filename=\""+filename+"\"");
			if(rs.next()){
				int end = rs.getInt("endindex");
				rs = stmt.executeQuery("select filename, endindex from fileclauselink where endindex<"+end+" order by endindex desc");
				int start = 0;
				if(rs.next()){
	...
	private String getDescription(int start, int end) throws SQLException {
		...
		for(int i = start; i <= end; i++){
			Statement stmt1 = conn.createStatement();
			ResultSet rs1 = stmt1.executeQuery("select clause, tag, attributes, modifier from clause where clauseid="+i);
			rs1.next();
			...
			starttag = starttag.replaceAll("^\\d+\\s*", "");
	...
	private String doSynonyms(int sentid, String sent){
		Pattern synonymsp = Pattern.compile(this.synonyms);
	...
	private void group(List terms, int clauseid, String matchedseg){
		...
		if(exp.compareTo("") != 0){
			...
			try{
				Statement stmt = conn.createStatement();
				ResultSet rs = stmt.executeQuery("select charsegment from sentence where sentid="+clauseid);
				rs.next();
			 ...
			 * try{
				Statement stmt = conn.createStatement();
				ResultSet rs = stmt.executeQuery("select charsegment from clause where clauseid="+clauseid);
				rs.next();
				String tmp =rs.getString("charsegment");
				if(tmp != null && tmp.compareTo("") !=0){
					matchedseg = tmp+";"+matchedseg; // seg#exp;seg#exp
				}
				stmt.execute("update clause set charsegment =\""+matchedseg+"\" where clauseid ="+clauseid);
			}catch (Exception e){
				e.printStackTrace();
			}*/
		}
	...
	private String add2LearnedStates(String term){
		...
		//if(term.indexOf("_")<0){ //normal term without "-"
			...
			try{
				Statement stmt =conn.createStatement();
				ResultSet rs = stmt.executeQuery("select state, count from learnedstates where state ='"+term+"'");
				if(rs.next()){
					int count = rs.getInt("count")+1;
					stmt.execute("update learnedstates set count ="+count+" where state ='"+term+"'");
				}else{
		...
		/*}else{
			...
			try{
				Statement stmt =conn.createStatement();
				ResultSet rs = stmt.executeQuery("select state, count from learnedstates where state ='"+t1+"'");
				if(rs.next()){//use t1
					int count = rs.getInt("count")+1;
					stmt.execute("update learnedstates set count ="+count+" where state ='"+t1+"'");
					t = t1;
				}else{//use t2
					rs = stmt.executeQuery("select state, count from learnedstates where state ='"+t2+"'");
					if(rs.next()){
						int count = rs.getInt("count")+1;
						stmt.execute("update learnedstates set count ="+count+" where state ='"+t2+"'");
					}else{
						stmt.execute("insert into learnedstates values('"+t2+"', 1)");
					}
					t = t2;
				}
			}catch (Exception e){
	...
	private String collectOrganNames(){
		...
		rs = stmt.executeQuery("select distinct tag from sentence");
		while(rs.next()){
		...
		//find pl. form
		rs = stmt.executeQuery("select word from wordpos where pos = \"p\"");
		while(rs.next()){
		...
		try{
			Statement stmt = conn.createStatement();
			ResultSet rs = stmt.executeQuery("select distinct tag from sentence order by tag");
			while(rs.next()){
				names.append(rs.getString("tag")+"|");
			}
			rs = stmt.executeQuery("select distinct word from wordpos where pos='p' order by word");
			while(rs.next()){
 ...
 */
	private void markSentences(){
		try{
			...
			/*moved to sentences2clauses
			 stmt.execute("create table if not exists clause (clauseid int(11) not null primary key, tag varchar(150), modifier varchar(150), clause varchar(500), charsegment varchar(250), attributes varchar(500))");
			stmt.execute("delete from clause");
			...
			*/
			Statement stmt = conn.createStatement();
			ResultSet rs = stmt.executeQuery("select count(sentid) from sentence");
			rs.next();
			...
			while(sentid < total){
				//rs = stmt.executeQuery("select tag, modifier, sentence from sentence where sentid="+sentid+""); Hong: 10/17/09

				rs = stmt.executeQuery("select tag, modifier, originalsent from sentence where sentid="+sentid+"");
				if(rs.next()){
	...
	private void createClauseTables(){
		try{
			Statement stmt = conn.createStatement();
			stmt.execute("create table if not exists clause (clauseid int(11) not null primary key, tag varchar(150), modifier varchar(150), clause varchar(500), charsegment varchar(500), attributes varchar(500))");
			stmt.execute("delete from clause");
			stmt.execute("create table if not exists fileclauselink (filename varchar(200) not null primary key, endindex int(11))");
			stmt.execute("delete from fileclauselink");
		}catch(Exception e){
	...
	private String[] getInfo(int sentid){
		...
		try{
			Statement stmt = conn.createStatement();
			ResultSet rs = stmt.executeQuery("select tag, modifier, charsegment from sentence where sentid="+sentid+"");
			rs.next();
	...
	private void addClause(int sentid, int clauseid, String tag, String modifier, String taggedclause, String charsegment, boolean lastclause){
		...
		try{
			Statement stmt = conn.createStatement();
			stmt.execute("insert into clause (clauseid, tag, modifier, clause, charsegment) values("+clauseid+", '"+tag+"', '"+modifier+"', '"+tmp+"', '"+charsegment+"')");
			if(lastclause){
				ResultSet rs = stmt.executeQuery("select filename from sentinfile where endindex="+sentid);
				if(rs.next()){
					String fname = rs.getString("filename");
					stmt.execute("insert into fileclauselink values ('"+fname+"', '"+clauseid+"')");
				}
			}
		}catch (Exception e){
			e.printStackTrace();
	...
	public static void main(String[] args) {
		...
		CharacterLearner cl = new CharacterLearner("fnav5_corpus",gfile);
		cl.markupCharState();
