...
public class StateCollector  {
	...
	StateCollector(String database){
		this.statematrix = new StateMatrix();
		//collect(database);
	}
	
	StateCollector(String database, ArrayList<String> knownstates){
		if(knownstates!=null){
			StateImporter si = new StateImporter(knownstates);
			this.statematrix = new StateMatrix(si.getStates());
		}
		//collect(database);
	}
	...
	protected void collect(String database){
		...
		// create statemartix
		StateCollector.database = database;
		try{
			if(conn == null){
...
*/
	...
	public static void main(String[] args) {
		...
		StateCollector sc = new StateCollector("fnav19_benchmark");
		sc.collect("test_asist09ont");
	}
