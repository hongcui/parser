...
public class V22_Transformer {
	...
	public static final String pheno="summer|winter|autumn|January|February|March|April|May|June|July|August|September|October|November|December|Sporulation|Sporulates|Sporulating|Sporophylls|Sporocarps|mature|Leaves|Spor|maturing|Flowering|Fruiting";
	...
	public static void main(String[] args) throws Exception{
		...
		V22_Transformer transformer = new V22_Transformer();
		//Select all the files from Volume 2 of FNA (FloraId of FNA = 1, VolumeId of Volume 2 =1002)
         String SQL = "select * from FloraTaxon where FloraId = 1 and VolumeId = 1022;";
...
private void processHabiEle(String text, int TaxonId) {
	...
	for(int a=0;a<textchunks.length;a++)
	{
		...
		else
		{
			...
			else if(localtext.matches(".*\\d+\\s*m\\s*")|localtext.matches(".*\\d+\\s*m"))//localtext.contains("elevation")
			{
				eletag=1;
			...
			else if(localtext.trim().matches(".*("+pheno+").*"))
			{
				...
				for(int b=0;b<phenohabi.length;b++)
				{
					...
					{
						if(phenohabi[b].contains("Fruiting"))
						{
							Element phe=new Element("phenology_fruiting");
 ...
 */
	private void processReference(String reftext) throws Exception{
		...
		String text="";
		/*reftext=cleaninputtext(reftext);
		String[] refchunks=reftext.split("<P>");*/
		...
		if(refchunks.length!=2)
		{
			...
			{
				String[] localchunks=refchunks[0].split("\\s");
				String heading = localchunks[0]+" "+localchunks[1];
				String localtext="";
				for(int c=2;c<localchunks.length;c++)
				{
					localtext+=" "+localchunks[c];
				}
				localtext=localtext.trim();
			    refchunks[0]=heading;
			    text=localtext;
			}
			else if(refchunks[1].matches(""))
			{
				refchunks[1]=refchunks[2];
				refchunks[2]="";
			}
		}
 ...
 */
   private void output(String TaxonName) throws Exception {
		...
		String file = "C:\\Users\\mohankrishna89\\Desktop\\Library Project\\V22\\Transformed\\" + TaxonName + ".xml";
   ...
   private void processName(String TaxonName,int TaxonId,int RankId) throws Exception{
	  ...
	  /* Element taxonname = new Element("TaxonName");
	   ...
	   else if((RankId==10042)) //It is a SubGenus
	   {
		   ...
		   String[] subgchunks = TaxonName.split("(Subg\\.)|(subg\\.)");
		   Element taxonname = new Element("genus_name");
	   ...
	   else if(TaxonName.contains("var.")) //It is a Variety
	   {
	   ...
	   else if((RankId==10032))//Subfamily name
	   {
		   taxname="subfamily_name";
		   int k;
			String famauth="";
			String subfamauth="";
			String[] family= TaxonName.trim().split("\\s+");
			Element newele= new Element("family_name");
			newele.setText(family[0]);
			taxonid.addContent(newele);
			...
			taxonid.addContent(subfamat);	
			}
			if(authorityname.length()!=0)
			   {
				   Element authname = new Element("subfamily_authority");
				   authname.setText(authorityname);
				   taxonid.addContent(authname);
			   }
	   }
	   else if((RankId==10035)) //Tribe Name
	   {
			int k;
			String spauth="";
			String sectauth="";
			String subgauth="";
			String[] var= TaxonName.trim().split("\\s+");
			Element newele= new Element("family_name");
		   ...
		   if(authorityname.length()!=0)
		   {
			   Element authname = new Element("tribe_authority");
			   authname.setText(authorityname);
			   taxonid.addContent(authname);
		   }
	   }
	   else if((RankId==10037))//Subtribe name
	   {
		   int k;
			String spauth="";
			String sectauth="";
		   ...
		   if(authorityname.length()!=0)
		   {
			   Element authname = new Element("subtribe_authority");
			   authname.setText(authorityname);
			   taxonid.addContent(authname);
		   }
	   }
   ...
   /*private String cleantext(String namepart) throws Exception{
		...
		result=inter.replace("&ouml;", "ö").replace("&eacute;", "é").replace("&auml;", "ä").trim();
		return result;
	}*/
   public String cleantext(String text) throws Exception{
		text=text.replaceAll("&amp;", "&").replaceAll(" ", " ").replaceAll("&eacute;", "é").replaceAll("&ouml;", "ö").replaceAll("&acirc;", "â").replaceAll("&ccedil;", "ç").replaceAll("&shy;", "").replaceAll("&ntilde;", "ñ")
				...
				.replaceAll("&oslash;", "ø").replaceAll("</P>", "").replaceAll("<BR>", "").replaceAll("&nbsp;", "").replaceAll("&Aacute;", "Á");	
		return text;
		}
   ...
   private Element synprocess(String namepart, String taxname) throws Exception{
		...
		else
		{
			...
			else if(text.contains("unranked"))//unranked name
			{
				int k;
				int unrank=0;
				String spauth="";
