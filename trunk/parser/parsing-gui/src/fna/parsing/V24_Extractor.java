...
public class V24_Extractor {
	...
	public static void main(String[] args) throws Exception {
		//int count = 1;
		V24_Extractor extractor = new V24_Extractor();
		...
		File extracted = new File("C:\\Users\\mohankrishna89\\Desktop\\FNA24\\target\\genus-extracted");
		File[] files = extracted.listFiles();
		for(int f = 1; f<=files.length; f++){
			SAXBuilder builder = new SAXBuilder();
			Document doc = builder.build("C:\\Users\\mohankrishna89\\Desktop\\FNA24\\target\\genus-extracted\\" + f + ".xml");
	...
	private void processParagraph(Element wp) throws Exception {
		...
		if(((text.matches("^\\d+\\w*\\.\\s*\\w+\\s+.*")&&(isbold||issmallcaps))||
				((text.matches("^\\d+\\w*\\..*(sect|subfam|var|subgen|subg|subsp|ser|tribe)\\..*\\d+\\s*\\.\\s*\\d+.*")&&isbold))||
				(text.matches("^\\d+\\w*\\..*(sect|subfam|var|subgen|subg|subsp|ser|tribe)\\..*")&&issmallcaps)||(text.matches("^\\d+\\w*\\.\\s*\\w+\\s*x?[A-Z]{2}[A-Z]*\\s.*"))||
				(text.matches("(^\\d+\\w*\\.\\s*)\\w+.*(\\[.*\\])"))||(text.matches("([A-Z][a-z]+\\s+).*((sect|subfam|var|subgen|subg|subsp|ser|tribe)\\.).*(\\[.*\\])"))||
				(text.matches("([A-Z][a-z]+\\s+)(\\w+).*(\\[.*\\])"))||(text.matches("([A-Z][a-z]+\\s+)(\\w+)\\s+[A-Z]\\.\\s(\\[.*\\]).*")&&isbold&&issmallcaps)||
	...
	private void output(int i) throws Exception {
		...
		String file = "C:\\Users\\mohankrishna89\\Desktop\\FNA24\\target\\Extracted\\" + i + ".xml";
